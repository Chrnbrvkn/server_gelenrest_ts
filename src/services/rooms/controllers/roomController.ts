...
export class RoomController {
    ...
    try {
      ...
      if (!roomId || !houseId) {
        throw new BadRequestError(
          `Не верный ID комнаты: ${roomId}. ID дома: ${houseId}`
        );
      }
      ...
      if (!room) {
        throw new NotFoundError(
          `Комната с ID: ${roomId} не найдена. ID дома: ${houseId}`
        );
      }
  ...
  static async deleteRoom(req: Request, res: Response, next: NextFunction) {
    ...
    if (!roomId || !houseId) {
      throw new BadRequestError(
        `Не верный ID квартиры: ${roomId}, ID дома: ${houseId}`
      );
    }
    ...
    try {
      ...
      if (!room) {
        ...
        throw new NotFoundError(
          `Комната с ID: ${roomId} не найдена. Room: ${room}`
        );
      }
        ...
        pictures.map(async (picture: RoomsPictures) => {
          ...
          if (!filename) {
            throw new InternalServerError(
              `Не удалось удалить картинку: ${picture.url}`
            );
          }
